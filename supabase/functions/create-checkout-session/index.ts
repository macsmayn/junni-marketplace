...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!secretKey) {
      console.error("[create-checkout-session] SUPABASE_SECRET_KEYS missing or 'default' entry not found");
    ...
    if (membershipErr) {
      console.error("[create-checkout-session] organization_members lookup failed:", membershipErr);
      return new Response(JSON.stringify({ error: "Internal error" }), {
        status: 500,
        headers: { ...CORS_HEADERS, "Content-Type": "application/json" },
      });
