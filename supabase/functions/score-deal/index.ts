...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!secretKey) {
      console.error("[score-deal] SUPABASE_SECRET_KEYS missing or 'default' entry not found");
...
- debt_principal_repayment: Check the Cash Flow Statement's Financing Activities section first — this is the authoritative location. Look for line items labeled "Repayment of Term Debt Principal," "Principal Repayment," "Repayment of Long-Term Debt," "Repayment of Bank Loan," "Payment on Term Loan," or any similar labeled principal outflow within that section. A clearly labeled Financing Activities line item is sufficient and authoritative on its own — you do NOT need corroborating text in the notes to extract it. Also check the notes when the cash flow statement is absent or the item does not appear there. Return as a POSITIVE number (even though the statement shows it in brackets or as a negative outflow). Null only when the documents contain no such line item.
...
- distributions: Check the Cash Flow Statement's Financing Activities section first — this is the primary location. Look for line items labeled "Dividends Paid," "Distributions Paid," "Dividends to Shareholders," "Dividends Declared and Paid," "Partner Distributions," "Owner Withdrawals," or any similar outflow to owners or equity holders within that section. A clearly labeled Financing Activities line item is sufficient on its own — you do NOT need a separate disclosure in the notes. Also check the notes and supplementary schedules. Return as a POSITIVE number. Null only when the documents show no such payment occurred during the year; do NOT return null based on the company's legal structure.
  ...
  "statements": [
    {
      ...
      "distributions": <number or null — cash paid to owners/shareholders/partners (dividends, draws, distributions); null only when no such payment occurred>,
